...
*   de ambiente LOCATEDB. Senão... Tchau.*)
  ...
  if (Registros.HL > 0) then
    Caminho := 'a:\utils\locate\db\';
  ...
  Caractere := ' ';
  if (VetorParametros[parametro] = '/A') or (VetorParametros[parametro] = '/CHANGE') then
    Caractere := 'A';
  if (VetorParametros[parametro] = '/C') or (VetorParametros[parametro] = '/COUNT') then
    Caractere := 'C';
  if (VetorParametros[parametro] = '/H') or (VetorParametros[parametro] = '/HELP') then
    LocateAjuda;
  if (VetorParametros[parametro] = '/L') or (VetorParametros[parametro] = '/LIMIT') then
    Caractere := 'L';
  if (VetorParametros[parametro] = '/P') or (VetorParametros[parametro] = '/PROLIX') then
    Caractere := 'P';
  if (VetorParametros[parametro] = '/S') or (VetorParametros[parametro] = '/STATS') then
    Caractere := 'S';
  if (VetorParametros[parametro] = '/V') or (VetorParametros[parametro] = '/VERSION') then
    LocateVersao;
  ...
  if (not (PrimeiraLetra in ['A'..'Z'])) then
    PrimeiraLetra := '0';
  ...
  if (Caractere in ['H', 'V', 'S', 'P', 'C']) then
    clrscr;

  (* Abre os arquivos de registros e de hashes *)
  if Caractere = 'P' then
    fastwriteln('Abre arquivo de registros');
  ArquivoRegistros := FileOpen(NomeArquivoRegistros, 'r');

  if Caractere = 'P' then
    fastwriteln('Abre arquivo de hashes');
